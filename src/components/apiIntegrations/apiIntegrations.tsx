...
const ApiIntegrations: FC<ApiIntegrationsProps> = ({
  apiKeys,
  deleteApiKey,
  generateApiKey,
  projects,
  ...
  return (
    <div
      className={classNames({
        "flex h-[100dvh] overflow-y-hidden items-center flex-col justify-center":
          apiKeys.length === 0,
      })}
    >
      <div>
        {apiKeys.length === 0 ? (
          <div className="mb-5 w-fit mx-auto">
            <p className="max-w-[297px] text-t-black/70 text-center text-sm">
              No API key has been generated. Click on the button to generate a
              new API key.
            </p>
          </div>
        ) : null}
        {apiKeys.length > 0 ? (
          <div className="mb-[85px]">
            <ApiKeys
              apiKeys={apiKeys}
              regenerateApiKey={generateApiKey}
